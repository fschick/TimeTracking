...
public class TimeSheetService : CrudModelService<TimeSheet, TimeSheetDto, TimeSheetGridDto>, ITimeSheetApiService
{
    ...
    /// <autogeneratedoc />
    public TimeSheetService(IDbRepository dbRepository, IMapper mapper, IFilterFactory filterFactory, IWorkdayService workdayService, IUserService userService, ISettingApiService settingService, IAuthorizationService authorizationService)
    ...
    /// <inheritdoc />
    public override async Task<TimeSheetDto> Get(Guid id, CancellationToken cancellationToken = default)
    {
        var result = await base.Get(id, cancellationToken);
        if (result == null)
            return null;

        if (!_authorizationService.CanView(result.UserId))
    ...
    public override async Task<List<TimeSheetGridDto>> GetGridFiltered(TimeSheetFilterSet filters, CancellationToken cancellationToken = default)
    {
        ...
        var timeSheets = await DbRepository
        ...
        await _userService.SetUserRelatedProperties(filters, timeSheets, cancellationToken);

        return timeSheets;
    }

    /// <inheritdoc />
    public override async Task<TimeSheetGridDto> GetGridItem(Guid id, CancellationToken cancellationToken = default)
    {
    ...
    {
        if (!_authorizationService.CanManage(dto.UserId))
            throw new ForbiddenException(ApplicationErrorCode.ForbiddenForeignUserData);
    ...
    {
        if (!await _authorizationService.CanManage<TimeSheetDto, TimeSheet>(dto))
            throw new ForbiddenException(ApplicationErrorCode.ForbiddenForeignUserData);
    ...
    {
        if (!await _authorizationService.CanManage<TimeSheet>(id))
            throw new ForbiddenException(ApplicationErrorCode.ForbiddenForeignUserData);
    ...
    public async Task<TimeSheetDto> StartSimilarTimeSheetEntry(Guid copyFromTimesheetId, DateTimeOffset startDateTime)
    {
        ...
        if (!_authorizationService.CanView(timeSheet.UserId))
            throw new ForbiddenException(ApplicationErrorCode.ForbiddenForeignUserData);
        ...
        timeSheetCopy.UserId = _authorizationService.CurrentUserId;
    ...
    public async Task<TimeSheetDto> StopTimeSheetEntry(Guid timesheetId, DateTimeOffset endDateTime)
    {
        ...
        if (!_authorizationService.CanManage(timeSheet.UserId))
            throw new ForbiddenException(ApplicationErrorCode.ForbiddenForeignUserData);
