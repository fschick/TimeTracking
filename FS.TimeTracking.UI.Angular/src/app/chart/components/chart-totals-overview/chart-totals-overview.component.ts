...
export class ChartTotalsOverviewComponent<TTotalRow extends TotalRow> {
  ...
  private createChartPlannedWorkedOptions(rows: TTotalRow[], type: 'planned' | 'worked'): DonutChartOptions {
    ...
    const label = type == 'planned'
      ? $localize`:@@Page.Chart.Common.Planned:[i18n] planned`
      : $localize`:@@Page.Chart.Common.Worked:[i18n] worked`;
  ...
  private reduceSeries(rows: TTotalRow[], valueSelector: (row: TTotalRow) => number): TTotalRow[] {
    ...
    series.push({
      customerTitle: $localize`:@@Page.Chart.Common.Other:[i18n] Other`,
      daysWorked,
      daysPlanned,
  ...
  private getToolTipChartPlannedWorked(seriesIndex: number, type: 'planned' | 'worked'): string {
    ...
    const label = type === 'planned'
      ? $localize`:@@Page.Chart.Common.DaysPlanned:[i18n] days planned`
      : $localize`:@@Page.Chart.Common.DaysWorked:[i18n] days worked`;
    ...
    return `
      <div class="d-flex">
        ...
        <div>
          ...
          <div>(${this.formatService.formatRatio(totalPercentage)} ${$localize`:@@Page.Chart.Common.Percentage:[i18n] %`} ${$localize`:@@Page.Chart.Common.OfTotalTime:[i18n] of total time`})</div>
        </div>
      </div>
    `;
  }
  ...
  private createChartLeftOptions(rows: TTotalRow[] | undefined): DonutChartOptions | undefined {
    ...
    const chartOptions = this.getDefaultChartOptions($localize`:@@Page.Chart.Common.Left:[i18n] left`, seriesIndex => this.getToolTipChartLeft(seriesIndex));
    ...
    chartOptions.plotOptions.pie!.donut!.labels!.total!.formatter = () => `${this.formatService.formatRatio(leftPercentage)}${$localize`:@@Page.Chart.Common.Percentage:[i18n] %`}`;
  ...
  private getToolTipChartLeft(seriesIndex: number): string {
    ...
    const label = title
      ? $localize`:@@Page.Chart.Common.DaysWorked:[i18n] days worked`
      : $localize`:@@Page.Chart.Common.WorkdaysLeft:[i18n] workdays left`;
